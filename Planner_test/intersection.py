    ...
    for t in interpolated_time:
        ...
        if t == interpolated_time[0]: # 第一列
            ...
            for s in interpolated_s:
                next_state = (t, s, 0, 0)
                J_cost, next_state = dp(cur_state=start_state, next_state=next_state, end_state=end_state, obs_state=obs_state, ref_velocity=1, v_max=v_max, v_min=v_min, a_max=a_max, a_min=a_min)
            ...
            for s in interpolated_s:
                ...
                for cur_state in state_matrix[:, i-1]:
                    if cur_state == None:
                        continue
                    next_state = (t, s, 0, 0)
                    J_cost, next_state = dp(cur_state=cur_state, next_state=next_state, end_state=end_state, obs_state=obs_state, ref_velocity=1, v_max=v_max, v_min=v_min, a_max=a_max, a_min=a_min)
