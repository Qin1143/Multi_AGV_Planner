...
def main():
    ...
    # s_d_ub = v_max
    [v_max, v_min, a_max, a_min] = [3.0, 0.0, 2.0, -2.0]
    dp_points_s, dp_points_t, end_state = dp_planner(path=path_one, dy_obs_in=dy_obs_in, dy_obs_out=dy_obs_out, v_max=v_max, v_min=v_min, a_max=a_max, a_min=a_min)
    # print(dp_points_s)
    start_state = (dp_points_t[0][0], dp_points_s[0][0], 0.0, 0.0)
    print(dp_points_s)
    osqp_result = qp_osqp(dp_points_t, dp_points_s, v_max, v_min, start_state=start_state, end_state=end_state, dy_obs_in=dy_obs_in, dy_obs_out=dy_obs_out)
    ...
    plt.plot(times, values, color='red', label='QP')
    ...
    plt.plot(time_points, displacement_points, marker='o', linestyle='-', color='blue', label='Global path')
    plt.plot(dp_points_t, dp_points_s, marker='o', color='purple', label='DP')
    if len(dy_obs_in) != 0:
        rectangle = Rectangle((3 - 0.5, 3 - 0.5), 1, 1, color='green', alpha=0.5, label='Obstacles')
